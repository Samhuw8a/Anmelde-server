from src.handling import Handler, Parser, User
from src.email_handler import Email_server
...
class Event_handler():
    def __init__(self)->None:
        self.parser              = Parser()
        self.settings:dict       = self.parser.load_settings("settings.json")
    ...
    def main(self)->None:
        ...
        if user.mail.strip()[-8:] not in self.trusted_mail_suffix:
            self.is_undone(user)
            raise Error("keine valide email adresse")
